...
export default class DefaultViewTable extends Component {
  render() {
    ...
    const renderItem = ({ item }) => {
      ...
      const isReported = !!(reports && reports.find(report => report === item.id))
      return (
        <DefaultViewTableCell
          key={item.id}
          editCell={editCell}
          isAdmin={isAdmin}
          item={item}
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        {userData.length > 2 ? (
          ...
          userData.map(item => renderItem({ item }))
        )}
        {data.length > 0 && (
          <View style={s.bottomListBox}>
            <FlatList
              data={data}
              keyExtractor={getId}
              ListFooterComponent={<View style={s.tableFooter} />}
              ref={ref => {
                this.bottomListRef = ref
              }}
              renderItem={renderItem}
            />
          </View>
  ...
  verifyData = isBottomTable => {
    ...
    if (isBottomTable) {
      const liveItems = newItems.filter(item => item.creator.id !== currentUser.id).sort(sortByDate)
      const resolvedItems = items.filter(item => item.isResolved).sort(sortByDate)
